...
function doQuery() {
    doGet(false, 'api/system_status/soe', function(err,httpResponse,response){
        ...
        if ( _.isNil(err) && !_.isNil(response) ) {
            ...
            module.exports.emit('soe-updated', percent)
        }
    })
    ...
    doGet(false, 'api/meters/aggregates', function(err,httpResponse,response){
        ...
        if ( _.isNil(err) && !_.isNil(response) ) {
            ...
            module.exports.emit('solar-updated', solar_power)
            module.exports.emit('grid-updated', grid_power)
            module.exports.emit('battery-updated', battery_power)
            module.exports.emit('load-updated', load_power)
        }
    })
}
